...
export async function runQoEAnalysisNonBlocking(processingInfo: JSONQoeProcessing) {
	...
	const files = await fsPromises.access(dir, fs.constants.R_OK | fs.constants.W_OK).then(() => fsPromises.readdir(dir));
...
async function runQoEAnalysis(processingInfo: JSONQoeProcessing, dir: string, files: string[], maxCpus?: number, onlyFiles = false) {
	...
	files.forEach((file) => {
		...
		promises.push(
			limit(() =>
				runSingleAnalysis(filePath, fileName, processingInfo, maxCpus)
					.then(async () => {
						if (!onlyFiles) {
...
async function runSingleAnalysis(filePath: string, fileName: string, processingInfo: JSONQoeProcessing, maxCpus?: number): Promise<string> {
	...
	return <Promise<string>>(
		runScript(
			`python3 ${process.env.PWD}/qoe_scripts/qoe_analyzer.py --presenter ${processingInfo.presenter_video_file_location} --presenter_audio ${processingInfo.presenter_audio_file_location} --viewer ${filePath} --prefix ${prefix} --fragment_duration_secs ${processingInfo.fragment_duration} --padding_duration_secs ${processingInfo.padding_duration} --width ${processingInfo.width} --height ${processingInfo.height} --fps ${processingInfo.framerate}` +
				maxCpusString
		)
	);
}
