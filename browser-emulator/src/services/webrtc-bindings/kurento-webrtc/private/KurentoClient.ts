...
export async function init(kurentoUrl: string): Promise<void> {
	console.log('[KurentoClient] Connect with Kurento Media Server:', kurentoUrl);
	try {
		...
		kurento.pipeline.on('Error', (event: any): void => {
			console.error('[KurentoClient] MediaPipeline ERROR %d (%s): %s', event.errorCode, event.type, event.description);
		});
	} catch (error) {
		throw error;
	}
...
export async function setPlayerEndpointPath(videoUri: string) {
	if (!kurento.player) {
		...
		console.log('[KurentoClient] Kurento PlayerEndpoint created, uri:', await kurento.player.getUri());

		kurento.player.on('Error', (event: any): void => {
			console.error('[KurentoClient] PlayerEndpoint ERROR %d (%s): %s', event.errorCode, event.type, event.description);
		});
		...
		kurento.player.on(
			...
			async (_event: any): Promise<void> => {
				if (kurento.player) {
					console.log('[KurentoClient] PlayerEndpoint End Of Stream: play() again');
...
export async function makeWebRtcEndpoint(recvonly: boolean = false, sendonly: boolean = false): Promise<any> {
	...
	try {
		...
		console.log(`[KurentoClient] Kurento WebRtcEndpoint created, recvonly: ${recvonly}, sendonly: ${sendonly}`);

		kurentoWebRtcEp.on('Error', (event: any): void => {
			console.error('[KurentoClient] WebRtcEndpoint ERROR %d (%s): %s', event.errorCode, event.type, event.description);
		});
		...
		if (!sendonly) {
			...
			if (kurento.recorderPathPrefix && recordingEnabled) {
				...
				const kurentoRecorder = await kurento.pipeline.create('RecorderEndpoint', {
					uri: `file://${kurento.recorderPathPrefix}.webm`,
					stopOnEndOfStream: true,
					mediaProfile: 'WEBM',
				});
				kurento.recorders.push(kurentoRecorder);
				console.log('[KurentoClient] Kurento RecorderEndpoint created, uri:', await kurentoRecorder.getUri());

				kurentoRecorder.on('Error', (event: any): void => {
					console.error('[KurentoClient] RecorderEndpoint ERROR %d (%s): %s', event.errorCode, event.type, event.description);
				});
				...
				console.log('[KurentoClient] WebRtcEndpoint connected to RecorderEndpoint');
			}
		}
		...
		if (APPLICATION_MODE === ApplicationMode.DEV) {
			...
			if (
				...
				console.log(`[KurentoClient] DEBUG: ${kurento.numWebRtcEndpoints} WebRtcEndpoints: print Pipeline DOT Graph!`);

				const pipelineDot: string = await kurento.pipeline.getGstreamerDot();

				try {
					await writeFile(`pipeline_${kurento.numWebRtcEndpoints}peers_${new Date().getTime()}.dot`, pipelineDot);
				} catch (err) {
